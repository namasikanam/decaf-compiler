...
/**
  ...
  */
class Parser(implicit config: Config)
    extends Phase[InputStream, Tree]("parser", config) {
    ...
    override def visitClassDef(ctx: DecafParser.ClassDefContext): ClassDef = {
      ...
      val parent =
        if (ctx.extendsClause != null)
          Some(ctx.extendsClause.id.accept(IdVisitor))
        else None
      val fields = ctx.field.map(_.accept(FieldVisitor))
      val mod =
        if (ctx.ABSTRACT != null)
          new Modifiers(Modifiers.ABSTRACT, getPos(ctx.ABSTRACT.getSymbol))
        else new Modifiers
      ClassDef(id, parent, fields, mod).setPos(getPos(ctx.CLASS.getSymbol))
    }
  }
  ...
  object FieldVisitor extends DecafParserBaseVisitor[Field] {
    ...
    override def visitMethodDef(ctx: DecafParser.MethodDefContext): Field = {
      ...
      val id = ctx.id.accept(IdVisitor)
      val params =
        if (ctx.varList == null) Nil
        else ctx.varList.`var`.map(_.accept(VarVisitor))
      val body =
        if (ctx.stmtBlock == null) Block(Nil)
        else ctx.stmtBlock.accept(StmtVisitor)
      val mod =
        if (ctx.STATIC != null) {
  ...
  object TypeLitVisitor extends DecafParserBaseVisitor[TypeLit] {
    ...
    override def visitIntType(ctx: DecafParser.IntTypeContext): TypeLit =
      positioned(ctx) { TInt() }

    override def visitBoolType(ctx: DecafParser.BoolTypeContext): TypeLit =
      positioned(ctx) { TBool() }

    override def visitStringType(ctx: DecafParser.StringTypeContext): TypeLit =
      positioned(ctx) { TString() }

    override def visitVoidType(ctx: DecafParser.VoidTypeContext): TypeLit =
      positioned(ctx) { TVoid() }

    override def visitClassType(ctx: DecafParser.ClassTypeContext): TypeLit =
      positioned(ctx) {
        TClass(ctx.id.accept(IdVisitor))
      }

    override def visitArrayType(ctx: DecafParser.ArrayTypeContext): TypeLit =
      positioned(ctx) {
        TArray(ctx.elemType.accept(this))
      }

    override def visitLambdaType(
        ctx: DecafParser.LambdaTypeContext
    ): TypeLit =
      positioned(ctx) {
        val returnType = ctx.`type`.accept(this)
        val typelist = ctx.typeList.`type`.map(_.accept(TypeLitVisitor))
        TLambda(returnType, typelist)
      }
  }

  object StmtVisitor extends DecafParserBaseVisitor[Stmt] {

    override def visitStmtBlock(ctx: DecafParser.StmtBlockContext): Stmt =
      positioned(ctx) {
    ...
    override def visitLocalVarDef(ctx: DecafParser.LocalVarDefContext): Stmt = {
      if (ctx.AUTO != null) {
        ...
        LocalVarDef(
          TVar(),
          id,
          Some(ctx.expr.accept(ExprVisitor)),
          getPos(ctx.ASSIGN.getSymbol)
        ).setPos(id.pos)
      } else {
    ...
    override def visitSkip(ctx: DecafParser.SkipContext): Stmt =
      positioned(ctx) { Skip() }
    ...
    override def visitIf(ctx: DecafParser.IfContext): Stmt = positioned(ctx) {
      ...
      val falseBranch =
        if (ctx.falseBranch == null) None
        else Some(blocked(ctx.falseBranch.accept(this)))
      If(cond, trueBranch, falseBranch)
    }

    override def visitWhile(ctx: DecafParser.WhileContext): Stmt =
      positioned(ctx) {
        val cond = ctx.cond.accept(ExprVisitor)
    ...
    override def visitBreak(ctx: DecafParser.BreakContext): Stmt =
      positioned(ctx) { Break() }

    override def visitReturn(ctx: DecafParser.ReturnContext): Stmt =
      positioned(ctx) {
        val expr =
          if (ctx.expr != null) Some(ctx.expr.accept(ExprVisitor)) else None
        Return(expr)
      }

    override def visitPrint(ctx: DecafParser.PrintContext): Stmt =
      positioned(ctx) {
  ...
  object LValueVisitor extends DecafParserBaseVisitor[LValue] {
    ...
    override def visitLValueIndex(
        ctx: DecafParser.LValueIndexContext
    ): LValue = {
  ...
  object ExprVisitor extends DecafParserBaseVisitor[Expr] {
    ...
    override def visitLiteral(ctx: DecafParser.LiteralContext): Expr =
      ctx.lit.accept(this)

    override def visitIntLit(ctx: DecafParser.IntLitContext): Expr =
      positioned(ctx) {
        IntLit(ctx.getText.toInt)
      }

    override def visitBoolLit(ctx: DecafParser.BoolLitContext): Expr =
      positioned(ctx) {
        BoolLit(ctx.getText.toBoolean)
      }

    override def visitStringLit(ctx: DecafParser.StringLitContext): Expr =
      StringLit(ctx.CLOSE_STRING.getText)
        .setPos(getPos(ctx.OPEN_STRING.getSymbol))

    override def visitNullLit(ctx: DecafParser.NullLitContext): Expr =
      positioned(ctx) { NullLit() }

    override def visitThis(ctx: DecafParser.ThisContext): Expr =
      positioned(ctx) { This() }

    override def visitParen(ctx: DecafParser.ParenContext): Expr =
    ...
    override def visitReadInt(ctx: DecafParser.ReadIntContext): Expr =
      positioned(ctx) { ReadInt() }

    override def visitReadLine(ctx: DecafParser.ReadLineContext): Expr =
      positioned(ctx) { ReadLine() }

    override def visitNewClass(ctx: DecafParser.NewClassContext): Expr =
      positioned(ctx) {
        NewClass(ctx.id.accept(IdVisitor))
      }

    override def visitNewArray(ctx: DecafParser.NewArrayContext): Expr =
      positioned(ctx) {
    ...
    override def visitClassTest(ctx: DecafParser.ClassTestContext): Expr =
      positioned(ctx) {
        val obj = ctx.expr.accept(this)
        val clazz = ctx.id.accept(IdVisitor)
        ClassTest(obj, clazz)
    ...
    override def visitExpressionLambda(
        ctx: DecafParser.ExpressionLambdaContext
    ): Expr = {
      val params =
        if (ctx.varList == null) Nil
        else ctx.varList.`var`.map(_.accept(VarVisitor))
      val expr = ctx.expr.accept(this)
      ExpressionLambda(params, expr).setPos(getPos(ctx.FUN.getSymbol))
    }

    override def visitBlockLambda(
        ctx: DecafParser.BlockLambdaContext
    ): Expr = {
      val params =
        if (ctx.varList == null) Nil
        else ctx.varList.`var`.map(_.accept(VarVisitor))
      val block = ctx.stmtBlock.accept(StmtVisitor)
      BlockLambda(params, block).setPos(getPos(ctx.FUN.getSymbol))
    }
  }
