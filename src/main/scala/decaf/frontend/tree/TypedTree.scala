...
object TypedTree extends TreeTmpl {
  ...
  /**
    ...
    * @param id Id
    */
  case class UnTypedNewClass(id: Id)(implicit val annot: ExprAnnot = NoType)
      extends Expr
  ...
  /**
    ...
    */
  case class UnTypedClassTest(obj: Expr, is: Id)(implicit val annot: ExprAnnot = NoType)
      extends Expr
  ...
  /**
    ...
    */
  case class UnTypedClassCast(obj: Expr, to: Id)(implicit val annot: ExprAnnot = NoType)
      extends Expr
  ...
  implicit def syntaxTreeExprAsUntyped(expr: SyntaxTree.Expr): UntypedExpr =
    UntypedExpr(expr).setPos(expr.pos)

  implicit def syntaxTreeExprListAsUntyped(
      exprs: List[SyntaxTree.Expr]
  ): List[UntypedExpr] =
    exprs.map { e =>
      UntypedExpr(e).setPos(e.pos)
    }

  implicit def syntaxTreeExprOptionAsUntyped(
      expr: Option[SyntaxTree.Expr]
  ): Option[UntypedExpr] =
    expr.map { e =>
      UntypedExpr(e).setPos(e.pos)
    }
